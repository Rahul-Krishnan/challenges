...
puts "\nNumber of people (excluding children): #{people.count}"
...
puts "\nAlia O\'Conner\'s phone number: #{people['Alia O\'Conner PhD']['phone']}"
# * How many children does Brian Heller have?
puts "\nBrian Heller has #{people['Brian Heller']['children'].count} offspring"
...
people.each do |name, details|
  puts name if !people[name]['children'].nil?
end
...
puts "\nBrian Heller's child is #{people['Brian Heller']['children'][0]}"
...
puts "\nCongrats to Dr. Adela DuBuque on her new baby, #{people['Dr. Adela DuBuque']['children'][0]}"
...
puts "\nThere are #{sets_of_people[0].count} people in the first set"
# * How many people are in the second set of people?
puts "\nThere are #{sets_of_people[1].count} people in the second set"
# * What is Ruby Hessel's phone number?
puts "\nRuby Hessel's phone number is #{sets_of_people[0]['Ruby Hessel']['phone']}"
# * What are the names of Dr. Sigrid Nader's children?
puts "\nDr. Sigrid Nader's children are:"
sets_of_people[1]['Dr. Sigrid Nader']['children'].each do |name|
  puts name
end
# * What is Bernard Feil's child's name?
puts "\nBernard Feil's child's name is #{sets_of_people[0]['Bernard Feil']['children'][0]}"
# * What company does Casandra Kemmer work for?
puts "\nCasandra Kemmer works for #{sets_of_people[0]['Casandra Kemmer']['company']}"
# * Who are all the people in the first set that have an `m` in their Company names?
puts "\nFirst set people with an 'm' in their company names:"
sets_of_people[0].each do |name, details|
  puts name if details['company'].downcase.include?('m')
end
